...
class AppSettings(BaseSettings):
    ...
    default_ai_provider: Literal["openai", "gemini", "perplexity", "groq"] = Field(
        default="groq", alias="DEFAULT_AI_PROVIDER"
    )
...
async def human_delay(min_s: float = 120, max_s: float = 600) -> None:
    wait = max(90, random.uniform(min_s, max_s))
...
async def read_delay(text: str) -> None:
    ...
    await asyncio.sleep(max(1.5, (words / max(wpm, 1)) * 60))
...
class RateLimiter:
    ...
    async def wait_if_needed(
        ...
        if not self._is_active_hours(active_hours_start, active_hours_end):
            wait_s = max(60, outside_sleep_min * 60)
        ...
        if hourly_cap > 0:
            ...
            if per_hour >= hourly_cap:
                ...
                logger.info(f"[Acc {account_id}] Hourly cap ({per_hour}/{hourly_cap}) - wait {wait / 60:.0f}min")
                await asyncio.sleep(wait)
        ...
        if burst_30min_cap > 0:
            ...
            if per_30 >= burst_30min_cap:
                ...
                logger.info(f"[Acc {account_id}] Burst cap ({per_30}/{burst_30min_cap}) - wait {wait / 60:.0f}min")
                await asyncio.sleep(wait)
